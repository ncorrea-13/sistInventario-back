...
import { crearArticulo, obtenerTodosLosArticulos, buscarArticuloPorId, actualizarArticulo, articuloStockSeguridad, articulosAReponer, definirProvDeterminado, ajusteInventario } from '../servicios/articuloServicio';
...
// POST para determinar el proveedor predeterminado de un artículo
router.get('/prov/:id', async (req, res) => {
  try {
    const codArticulo = Number(req.params.id);
    const provId = Number(req.body.provId);
    const resultado = await definirProvDeterminado(codArticulo, provId);
    res.status(200).json({ resultado });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: 'Error al obtener el proveedor predeterminado' });
  }
});
