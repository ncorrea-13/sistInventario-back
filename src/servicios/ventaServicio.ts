...
export const crearVenta = async (data: { montoTotalVenta: number; articulos: [{ articuloId: number; cantidad: number; }]; }) => {
  ...
  for (const articulo of data.articulos) {
    try {
      ...
      if (articulo.cantidad > articuloStock.stockActual) {
        throw new Error(`No hay suficiente stock para el artículo con ID ${articulo.articuloId}. Stock actual: ${articuloStock.stockActual}, cantidad solicitada: ${articulo.cantidad}`);
      }
    } catch (error) {
  ...
  for (const articulo of data.articulos) {
    try {
      ...
      const articuloStock = await prisma.articulo.update({
        where: { codArticulo: articulo.articuloId },
        data: {
          stockActual: {
